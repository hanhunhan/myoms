...
class finalaccounts extends   FlowBase{
	...
	public function handleworkflow($data){//下一步
		...
		if ($str) {
			$this->model->commit();
			//$this->UserLog->writeLog($data['flowId'],"__APP__","项目决算流程办理成功");
			...
			$this->model->rollback();
			//$this->UserLog->writeLog($data['flowId'],"__APP__","项目决算流程办理失败");
			//js_alert('办理失败');
			$result = -2;
	...
	public function passWorkflow($data){//确定
		...
		if ($str) {
			$this->model->commit();
			//$this->UserLog->writeLog($data['flowId'],"__APP__","项目决算流程同意成功");
			...
			$this->model->rollback();
			//$this->UserLog->writeLog($data['flowId'],"__APP__","项目决算流程同意失败");
			//js_alert('同意失败');
			$result = -3;
	...
	public function notWorkflow($data){//否决
		...
		if ($str) {
			...
			if($res){
				$this->model->commit();
				//$this->UserLog->writeLog($data['flowId'],"__APP__","项目决算流程否决成功");
				...
				$this->model->rollback();
				//$this->UserLog->writeLog($data['flowId'],"__APP__","项目决算流程否决失败 数据操作失败");
	...
	public function finishworkflow($data){//备案
		...
		if ($str) {
			...
			if($ress){
				...
				//$this->UserLog->writeLog($data['flowId'],"__APP__","项目决算流程备案成功");
				//if($this->cType=='pc')  js_alert('备案成功', U('Flow/already'));
				//else js_alert('备案成功' );
				$result = 5;
			}else{
				$this->model->rollback();
				//$this->UserLog->writeLog($data['flowId'],"__APP__","项目决算流程备案失败  数据操作失败");
			...
			if ($recordId) {
				...
				if ($fstatus == 0 || $fstatus == 3) {
					...
					if ($str) {
						...
						if($ress){
							$this->model->commit();
							//$this->UserLog->writeLog($data['flowId'],"__APP__","项目决算流程提交成功");
							//if($this->cType=='pc') js_alert('提交成功', U('Case/opinionFlow_final',  $data['url_param'])); else js_alert('提交成功');
							$result = 6;
						}else{
							$this->model->rollback();
							//$this->UserLog->writeLog($data['flowId'],"__APP__","项目决算流程提交失败 数据操作失败!");
	...
	private  function costPosArr($data,$fee){
		$cost_info = array();
		$cost_info['CASE_ID'] = $data["CASE_ID"];            //案例编号 【必填】
		$cost_info['ENTITY_ID'] = $data['ID'];                 //业务实体编号 【必填】
		$cost_info['EXPEND_ID'] = $data['ID'];                  //成本明细编号 【必填】
		$cost_info['ORG_ENTITY_ID'] = $data['ID'];                  //业务实体编号 【必填】
		...
		$cost_info['FEE_REMARK'] = "POST机手续费-决算";             //费用描述 【选填】
		$cost_info['INPUT_TAX'] = 0;                                //进项税 【选填】
		$cost_info['FEE_ID'] = 95;                                  //成本类型ID 【必填】支付第三方费用
		$cost_info['EXPEND_FROM'] = 28;                             //成本来源
	...
	private  function costTaxArr($data,$fee){
		...
		$cost_info['ISFUNDPOOL'] = 0;                                 //是否资金池（0否，1是） 【必填】
		$cost_info['ISKF'] = 1;                                     //是否扣非 【必填】
		$cost_info['FEE_REMARK'] = "税金-决算";             //费用描述 【选填】
		$cost_info['INPUT_TAX'] = 0;                                //进项税 【选填】
		$cost_info['FEE_ID'] = 96;                                  //成本类型ID 【必填】支付第三方费用
		$cost_info['EXPEND_FROM'] = 29;                             //成本来源
