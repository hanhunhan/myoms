    ...
    public function __call($method,$args) {
        if( 0 === strcasecmp($method,ACTION_NAME)) {
            ...
            if(isset($input[$args[0]])) { // 取值操作
				...
				//$data = mb_detect_encoding($data, 'UTF-8') === 'UTF-8' ? iconv('UTF-8','GBK', $data ) : $data; 
               // $fun  =  $args[1]?$args[1]:C('DEFAULT_FILTER');
				//$data = mb_detect_encoding($data,array('ASCII','GB2312','GBK','UTF-8')) == 'UTF-8' ?  $fun($data, ENT_NOQUOTES ,'UTF-8') : $data ;//$fun($data, ENT_NOQUOTES, "GB2312")
                //$data	 =	 $fun($data, ENT_NOQUOTES, "gb2312"); // 参数过滤
				$ac = mb_detect_encoding($data,array('ASCII','GB2312','EUC-CN','GBK','UTF-8'));  
				if(!in_array(strtoupper($ac),array('ASCII','GB2312','EUC-CN','GBK') ) ){
					$data = iconv('UTF-8','GB2312',$data);//ajax只能传uft-8
				}
            }else{ // 变量默认值
