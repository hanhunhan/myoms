		...
		if($this->TRANSFER && $this->DBBOUND==-1){
			...
			switch($this->EDITTYPE){
				case 1://文本
					 ...
					 $INPUTHTML.='<td '.$GRIDTD.$attr.'><span class="fclos '.$spanfirst.'" style="display:block;width:100%;overflow: hidden;white-space:nowrap;">'.$defaultValue.'</span><span class="fclos '.$spansecond.'"><input '.$inputproperty.' type="text" class="form-control" name="'.$fieldName.'"  value="'.$defaultValue.'" size="'.$this->EDITLENGTH.'" maxlength="'.$this->EDITMAXLENGTH.'" /></span>'.$orivalue .$this->UNIT.'<input type="hidden" value="'.$defaultValue.'" name="'.$fieldName.'_OLD" />'.'</td>';

					break;
				case 12://数字
					$orivalue = !is_null($param['ISNEW'])?'<span class="fclos fred">'.$versionC.''.$param['ORIVALUEE'].'</span>':'';
					 $inputproperty .= $CHKADD ? '' : $FIELDTYPE ;
					 $INPUTHTML.='<td '.$GRIDTD.$attr.'><span class="fclos '.$spanfirst.'">'.$defaultValue.'</span><span class="fclos '.$spansecond.'"><input '.$inputproperty.' type="text" class="form-control" size="'.$this->EDITLENGTH.'" name="'.$fieldName.'" value="'.$defaultValue.'" maxlength="'.$this->EDITMAXLENGTH.'" /></span>'.$orivalue.$this->UNIT.'<input type="hidden" value="'.$defaultValue.'" name="'.$fieldName.'_OLD" />'.'</td>';
					break;
				case 13://日期编辑
					$orivalue = !is_null($param['ISNEW'])?'<span class="fclos fred">'.$versionC.''.$param['ORIVALUEE'].'</span>':'';
					 $inputproperty .= $CHKADD ? '' : $FIELDTYPE  ;
				...
				case 5://textarea
					 ...
					 $INPUTHTML.='<td '.$GRIDTD.$attr.'><span class="fclos '.$spanfirst.'" style="display:block;width:150px;overflow: hidden;white-space:nowrap; ">'.$defaultValue.'</span><span class="fclos '.$spansecond.'"><textarea '.$inputproperty.' cols="'.$this->EDITLENGTH.'" rows="'.$rows.'" maxlength="'.$this->FIELDLENGTH.'"  name="'.$fieldName.'" >'.$defaultValue.'</textarea></span>'.$orivalue.$this->UNIT.'<input type="hidden" value="'.$defaultValue.'" name="'.$fieldName.'_OLD" />'.'</td>';
				...
				case 7:
					...
					break;
				case 77:
					$INPUTHTML .='<td '.$GRIDTD.$attr.' style="color:#FF9999; "><input class="form-control" type="text" name="'.$fieldName.'"  value="'.$defaultValue.'" size="'.$this->EDITLENGTH.'" maxlength="'.$this->EDITMAXLENGTH.'"  autocomplete="off"   plugin="swfupload" class="form-control" /> <span id="spanButtonPlaceholder"></span>   '.$this->HELPTEXT.'  <input type="hidden" pluginhidden="swfupload" name="hidFileID" id="hidFileID" value="" /> <div id="thumbnails" filedname="'.$fieldName.'"  dfvalue="'.$defaultValue.'">
					<table id="infoTable" border="0"  style="width:50%;display:block; border: solid 1px #7FAAFF; background-color: #C5D9FF; padding: 2px;margin-top:8px;">
					</table>
				</div> </td>';
					break;
