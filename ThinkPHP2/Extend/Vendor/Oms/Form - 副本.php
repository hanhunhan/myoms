...
class Form{
	...
	*/
	...
	public function getFormHtml(){
			...
			if(count($val)==1 && $maxColno>1){
					...
					if($result){
						...
						if($this->changeRecord && $this->changeRecordVersionId && $changarr[$vall->FIELDNAME]){
							...
							$param['ORIVALUE'] = $changarr[$vall->FIELDNAME]['ORIVALUEE']; 
		...
		$bhtml = $this->FormeditType!=2 ? '<input type="submit" value="保&nbsp;存" class="btn-blue" /> <input type="button" value="关&nbsp;闭" class="btn-gray j-formclose" /> ' :'';
	...
	public function getGridHtml(){
		...
		foreach($rows as $key=>$val){
			...
			if($this->CZBTN){
				...
				$trs .= '<td class="fedit" fid="'.$val[$this->PKFIELD].'"> ';
				$trs .= $czbtn;
				$trs .= '</td></tr>';

			}else{
				$formUrl =$this->joinUrl(__SELF__,'showForm=1&'.$this->PKFIELD.'='.$val[$this->PKFIELD] ) ;
				$formUrl2 =$this->joinUrl(__SELF__,'showForm=2&'.$this->PKFIELD.'='.$val[$this->PKFIELD] ) ;
    ...
    public function getPage($totalRecords,$page,$pageSize,$pages){
		...
		$bhtml = $this->GCBTN ? $this->GCBTN : $bhtml;
		$bhtml .= $this->GABTN;
	...
	public function getStatusTable($valarr,$bjarr){
		...
		$script =  '<script> var jsondata =  '.json_encode($data2).' ; 	$(document).ready(function(){';
