...
function U($url,$vars='',$suffix=true,$redirect=false,$domain=false) {
    ...
    // ????????
    if(is_string($vars)) { // aaa=1&bbb=2 ?????????
        parse_str($vars,$vars);
    }elseif(!is_array($vars)){
        $vars = array();
    }
    if(isset($info['query'])) { // ?????????????? ?????vars
...
function session($name,$value='') {
    ...
    if(is_array($name)) { // session????? ??session_start ??????
        ...
        }elseif($prefix){ // ???session
            return $_SESSION[$prefix][$name];
        }else{
            return $_SESSION[$name];
        }
    }elseif(is_null($value)){ // ???session
        if($prefix){
...
function cookie($name, $value='', $option=null) {
    ...
    $config = array(
        ...
        'domain' => C('COOKIE_DOMAIN'), // cookie ??ßπ????
    );
    // ????????(?????a??????)
    if (!empty($option)) {
