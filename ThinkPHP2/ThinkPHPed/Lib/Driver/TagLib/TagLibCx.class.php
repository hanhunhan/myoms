...
class TagLibCx extends TagLib {
    ...
    protected $tags   =  array(
        // ??????âÿ attr ?????ß“? close ??????0 ????1 ???1?? alias ??????? level ?????
        'php'=>array(),
    ...
    public function _volist($attr,$content) {
        ...
        // ??????®≤????⁄Ö????? <volist name=":fun('arg')" id="vo">{$vo.name}</volist>
        $parseStr   =  '<?php ';
        if(0===strpos($name,':')) {
    ...
    /**
     +----------------------------------------------------------
     * compare???????
     * ????????? ??? eq neq gt lt egt elt heq nheq ?????eq
     * ????? <compare name="" type="eq" value="" >content</compare>
     +----------------------------------------------------------
     * @access public
     +----------------------------------------------------------
     * @param string $attr ???????
     ...
     * range???????
     * ?????????????????????¶∂ ????????? type= in ??????¶∂?? ?????????¶∂??
     * ????? <range name="var|function"  value="val" type='in|notin' >content</range>
    ...
    /**
     +----------------------------------------------------------
     * import ??????? <import file="Js.Base" /> <import file="Css.Base" type="css" />
     +----------------------------------------------------------
     * @access public
     +----------------------------------------------------------
     * @param string $attr ???????
