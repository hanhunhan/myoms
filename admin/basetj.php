...
if($_REQUEST['action']=='gettongji'){
	...
	if($_REQUEST['itemid']){
		$itemid = implode(',',getChildrens($db,$_REQUEST['itemid']) );
	  	$sql = "SELECT * FROM ".$DB_PREFIX."new_user a left join ".$DB_PREFIX."new_user_result b  on a.USER_ID=b.uid where a.DEPT_ID in($itemid) and a.needsend=1   ";
	}else $sql = "SELECT * FROM ".$DB_PREFIX."new_user a left join ".$DB_PREFIX."new_user_result b  on a.USER_ID=b.uid  where a.needsend=1  ";
...
三六五网邀请您参加组织氛围在线调研的时间已接近尾声。<br> 
 请您点击以下地址进入调研平台（仅支持PC端，暂不支持手机作答）：<a href='$urll' target='_blank'> $urll</a> <br> 
 
 这是您的专属调研地址，与您的信息绑定，请勿转发给他人。调研团队承诺，将严格保证本次调查的匿名性和数据准确性。<br> 
 如果无法打开上面的链接，请进入组织氛围OA通栏直接作答。<br> 
 建议您使用火狐浏览器或IE9以上的浏览器进行作答。<br> 
 本次测试邀请于<font color="#ff0000"> $startime</font> 生效，于 <font color="#ff0000"> $endtime</font> 失效。请抓紧时间完成调研。<br> 
 三六五网 <br> 
$sendtime <br>
...
eod2;
		oa_notice($_REQUEST['touid'], $_REQUEST['touid'], $subject, $content);
		send_sms($msg,$user['phone'],'nj');
...
<body>
  ...
  <table id="tj" class="easyui-datagrid" title="组织氛围调研各业务实体各题样本分布及分值情况表" style="width:98%;height:auto"
			data-options="rownumbers:true,singleSelect:true,url:'basetj.php?action=gettongji',method:'get',toolbar:'#tb',pagination:true,pageSize:30">
		<thead>
			<tr>
				...
				<th data-options="field:'other',width:100,align:'left',formatter:formatCellTooltip"> 其他意见</th>
