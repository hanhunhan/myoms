...
/**
 * submit_crm_data_by_api_url
 *
 * 提交CRM接口数据
 *
 * @access  public
 * @param   array   $api_arr 接口需要提交的数据
 * @return  string   提交的链接地址
 */
function submit_crm_data_by_api_url($api_arr)
{
	$crm_api_url = P_CRM.'index.php/Simulate/sea?';
...
function get_userinfo_by_pid_telno($project_id, $telno, $pro_listid, $city_spell = 'nj')
{
	...
	//$userinfo_crm_arr = get_crm_userinfo_by_pid_telno($project_id, $telno, $city_spell);  不在调用CRM的接口
	if(is_array($userinfo_crm_arr) && $userinfo_crm_arr['result'] == 1 &&
...
function get_fgj_userinfo_by_code($code)
{
	...
	$userinfo_fgj_json = curl_get_contents($api_fgj , 'get');
	$userinfo_fgj_arr = json_decode($userinfo_fgj_json , TRUE);
    //var_dump($userinfo_fgj_json);
    //var_dump($userinfo_fgj_arr);die;
	return $userinfo_fgj_arr;
}
 ...
 */
function member_opreate_log($log_info)
{   
    $add_result = FALSE;
    
    if(!empty($log_info) && is_array($log_info))
    {
       $add_result =  M('erp_cardmember_op_log')->add($log_info);
    }
...
/**
 * submit_user_operate_log
 *
 * 提交
 *
 * @access  public
 * @param   int   $user_id 被操作用户id
 * @param   int  $operate_type 操作类型 1办卡 ，2状态修改
 * @param   string   $operate_remark 操作方式描述
 * @param   string  $operate_user 操作人
 * @param   int   $from_device 操作人设备类型 1安卓设备，2、IOS设备，3、PC
 * @return  boolean 是否提交成功
 */
function submit_user_operate_log($user_id, $operate_type, $operate_remark, $operate_user, $from_device, $city_id , $prjid = 0)
{
	$isert_data = array();
	$isert_data['USER_ID'] = intval($user_id);
	$isert_data['OPERATE_TYPE'] = intval($operate_type);
	$isert_data['OPERATE_REMARK'] = strip_tags($operate_remark);
	$isert_data['OPERATE_USER'] = strip_tags($operate_user);
	$isert_data['OPERATE_TIME'] = date("Y-m-d H:i:s");
	$isert_data['FROM_DEVICE'] = intval($from_device);
	$isert_data['IP'] = ip2long(get_client_ip());
	$isert_data['PRJID'] = intval($prjid);
	$isert_data['CITY'] = intval($city_id);

	$add_result =  M('erp_rt_operate_log')->add($isert_data);
 ...
 *
 * @access  public
 * @param   int  $customer_id   客户编号
 * @param   int  $truename      签到用户姓名
 * @param   string  $telno      签到用户手机号码
 * @param   int  $code       签到验证码
 * @param   int  $project_id 签到楼盘ID
 * @param   int  $project_listid 通用楼盘ID
 * @param   int  $is_from     签到用户来源，1CRM，2房管家
 * @param   int  $is_confirm  是否已确定签到
 * @return  boolean  添加日志是否成功
 */
function arrival_confirm_log($customer_id, $truename, $telno, $code,
							 $project_listid, $project_id, $is_from, $is_confirm)
{

	$isert_data = array();
	$isert_data['CUSTOMER_ID'] = intval($customer_id);
	$isert_data['PROJECT_ID'] = intval($project_id);
	$isert_data['NEWHOUSE_ID'] = intval($project_listid);
	$isert_data['TRUENAME'] = u2g(strip_tags($truename));
	$isert_data['TELNO'] = strip_tags($telno);
	$isert_data['CODE'] = intval($code);
	$isert_data['IS_FROM'] = strip_tags($is_from);
	$isert_data['IS_CONFIRM'] = intval($is_confirm);
	$isert_data['CREATETIME'] = date("Y-m-d H:i:s");

	if(!empty($isert_data))
	{
		$add_result =  M('erp_arrival_confirm_log')->add($isert_data);
	}
