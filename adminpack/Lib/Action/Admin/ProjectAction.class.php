	...
	class ProjectAction extends ExtendAction{
        ...
        */
        ...
        {   
			...
			$scale_type =  urldecode($this->_request('case_type')) ;
            
           // $case_type =  'ds';
            $conf_case_type = $project_case_model->get_conf_case_type();
            $scale_type = intval($conf_case_type[$scale_type]);
            $project_info = $project->get_my_project_list($uid, $scale_type, $search_key , $city);
            ...
            {
                $ajax_data[0]['id'] = 0;
                $ajax_data[0]['city_id'] = 0;
                $ajax_data[0]['label'] =  g2u('未找到符合条件的项目');  
            }
        ...
        public function asyncGetDSProjects() {
            $city = $this->channelid;
            //根据关键词获取楼盘信息
            ...
            $sql = "
                SELECT *
                FROM ERP_PROJECT T
                WHERE T.PROJECTNAME LIKE '%{$search_key}%'
                  AND T.STATUS <> 2
				  AND T.PSTATUS = 3
                  AND T.CITY_ID = {$city}
                  AND T.ID IN
                    (SELECT PROJECT_ID
                     ...
                     WHERE C.SCALETYPE = 1)
            ";
            $projectList = D('Project')->query($sql);
            if (is_array($projectList) && count($projectList)) {
                foreach($projectList as $key => $val) {
                    $ajaxData[$key]['id'] = $val['ID'];
                    $ajaxData[$key]['city_id'] = $val['CITY_ID'];
                    $ajaxData[$key]['label'] = g2u($val['PROJECTNAME']);
                }
            } else {
                $ajaxData[0]['id'] = 0;
                $ajaxData[0]['city_id'] = 0;
                $ajaxData[0]['label'] = g2u('未找到符合条件的项目');
            }
        ...
        public function ajax_get_feescale()
        {
            ...
            if(!empty($feescale) && is_array($feescale))
            {
                foreach ($feescale as $key => $value)
                {
					...
					$ajax_data[$key]['MTYPE'] = $value['MTYPE'];
					$ajax_data[$key]['STYPE'] = $value['STYPE'];
					if ($value['AMOUNT'][0] == '.') {
						$ajax_data[$key]['AMOUNT'] = '0' . $value['AMOUNT'];
					}else{
         ...
         * @return json 费用数据
         +----------------------------------------------------------
         */
        public function ajax_get_fx_feescale()
        {	
            //项目编号
            $prj_id =  intval($this->_request('prj_id'));
            ...
            {	
                ...
                {	
                    $case_id = !empty($caseinfo[0]['ID']) ? intval($caseinfo[0]['ID']) : 0;
                    $project = D('Project');
                    $ajax_data['dtsf_front'] = $project->get_feescale_by_cid($case_id, 1, $status,0)?1:0;
					$ajax_data['dtsf_after'] = $project->get_feescale_by_cid($case_id, 1, $status,1)?1:0;
					$ajax_data['zjyj_front'] = $project->get_feescale_by_cid($case_id, 2, $status,0)?1:0;
					$ajax_data['zjyj_after'] = $project->get_feescale_by_cid($case_id, 2, $status,1)?1:0;
                }
            }
		...
		public function upload_files(){
			if($_REQUEST['act'] == "save"){
				...
				$info['msg'] = "";
				if($_REQUEST['prjid']){
					$data['FILES'] = u2g($_REQUEST['file']);
					$res = M("Erp_house")->where("PROJECT_ID=".$_REQUEST['prjid'])->save($data);
					if($res){
						$info['status'] = 1;
						$info['msg'] = "保存成功";
						echo json_encode(g2u($info));
						exit;
					}
				}
				echo json_encode(g2u($info));
			...
			$projectId = $_REQUEST['prjid'] ? $_REQUEST['prjid'] : 0 ;
			if($projectId){
				$project = M("Erp_house") ->where("PROJECT_ID=".$projectId)->find();
			}
			$this->assign('attach',$project['FILES']);
			$this->assign('projectName',$project['PRO_NAME']);
			$this->assign('prjId',$projectId);
			$this->display('upload_files');
		}
	}
?>