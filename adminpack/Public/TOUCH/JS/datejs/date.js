...
(function ($) {
    $.fn.date = function (options,Ycallback,Ncallback) {
        ...
        function init_iScrll() {
            ...
            var strD = $("#monthwrapper ul li:eq("+indexD+")").html();

            yearScroll = new iScroll("yearwrapper",{snap:"li",vScrollbar:false,
                onScrollEnd:function () {
                    $("#yearwrapper ul li:eq("+indexY+")").css("color","#4c4c4c");

                    indexY = (this.y/40)*(-1)+1;
                //    opts.endday = checkdays(strY,strM);
                 //   $("#daywrapper ul").html(createDAY_UL());
                    yearScroll.refresh();
                    $("#yearwrapper ul li:eq("+indexY+")").css("color","#41a0e0");
                }});
            monthScroll = new iScroll("monthwrapper",{snap:"li",vScrollbar:false,
                onScrollEnd:function (){
                    $("#monthwrapper ul li:eq("+indexM+")").css("color","#4c4c4c");
                    indexM = (this.y/40)*(-1)+1;

                 //   opts.endday = checkdays(strY,strM);
                //    $("#daywrapper ul").html(createDAY_UL());
                    monthScroll.refresh();
                    $("#monthwrapper ul li:eq("+indexM+")").css("color","#41a0e0");
                }});
            dayScroll = new iScroll("daywrapper",{snap:"li",vScrollbar:false,
                onScrollEnd:function () {
                   // alert(indexD)
                    $("#daywrapper ul li:eq("+indexD+")").css("color","#4c4c4c");
                    indexD = (this.y/40)*(-1)+1;
               //    opts.endday = checkdays(strY,strM);
                //   $("#daywrapper ul").html(createDAY_UL());
                    dayScroll.refresh();
         //   var aa=Math.floor(indexD);;
            $("#daywrapper ul li:eq("+indexD+")").css("color","#41a0e0");
                   // indexD = aa;
        }});
        }
