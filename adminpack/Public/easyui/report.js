...
(function($){
	...
	function getColumns(target, data){
		...
		var opts = $.data(target, 'pivotgrid').options;
		var columns = [];  
		if(opts.pivot.columns!=''){//
			$.map(opts.pivot.columns, function(field, index){
				var pcolumns = columns[index-1]; //console.log(pcolumns);
				if (pcolumns){
					var cc = [];
		...
		if (!state.layoutDialog){
			...
			state.layoutDialog.dialog({
				...
				buttons:[{
					...
					width:80,
					handler:function(){
						opts.pivot.filters =  getFields('filters');//console.log(opts.pivot.filters);
						...
						$(target).pivotgrid();
						$.post(location.href+'&saveLayout=1',{'filters':opts.pivot.filters,'rows':opts.pivot.rows,'columns':opts.pivot.columns,'values':opts.pivot.values},function(d){ });///
					}
				},{
